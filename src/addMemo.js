...
import { findUnusedOrdersByPrice, markOrderAsUsed } from './db.js';
config();
...
async function getUnapprovedTransactions() {
	try {
		const response = await ynabAPI.transactions.getTransactions(budgetId, undefined, 'unapproved');
...
function findAmazonTransactionsWithoutMemos(transactions) {
	return transactions.filter((txn) => {
		...
		const isAmazon = txn.payee_name && txn.payee_name.toLowerCase().includes('amazon');
...
async function updateTransactionMemo(transactionId, memo) {
	try {
		const txnResponse = await ynabAPI.transactions.getTransactionById(budgetId, transactionId);
		...
		const response = await ynabAPI.transactions.updateTransaction(budgetId, transactionId, { transaction: transactionUpdate });
...
export async function addMemos() {
	try {
		...
		console.log(`Found ${amazonTransactions.length} unapproved Amazon transactions without memos`);
		...
		for (const txn of amazonTransactions) {
			...
			console.log(`Processing transaction: $${txnAmount.toFixed(2)} on ${txn.date}`);

			// Find matching orders in the database
			const matchingOrders = await findUnusedOrdersByPrice(txnAmount);

			if (matchingOrders.length === 0) {
				console.log(`No matching Amazon orders found for $${txnAmount.toFixed(2)}`);
				continue;
			}
